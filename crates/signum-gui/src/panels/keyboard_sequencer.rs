...
impl KeyboardSequencerPanel {
    ...
    pub fn get_drum_steps(&self) -> &[bool] {
        &self.drum_steps
    }
    ...
    pub fn ui(
        ...
        if is_playing && sample_rate > 0 {
            ...
            self.current_step = (elastic_beat as usize) % sc;
        }
        ...
        self.drum_steps.resize(sc, false);
        ...
        ui.vertical(|ui| {
            self.draw_drum_row(ui, is_playing);
            ui.add_space(sp);
    ...
    fn handle_drum_input(&mut self, ui: &mut Ui) -> Vec<KeyboardSequencerAction> {
        ...
        for (i, &key) in DRUM_KEYS[..sc].iter().enumerate() {
            ...
            self.drum_steps[i] = !self.drum_steps[i];
    ...
    fn handle_melodic_input(&mut self, ui: &mut Ui) -> Vec<KeyboardSequencerAction> {
        ...
        let velocity = ui.input(|i| {
            ...
            if i.modifiers.ctrl { return 40u8; }
            self.base_velocity
        });
    ...
    fn draw_drum_row(&self, ui: &mut Ui, is_playing: bool) {
        let l = self.layout();
        ui.horizontal(|ui| {
            ...
            for (i, &active) in self.drum_steps.iter().enumerate() {
                ...
                self.draw_pad(&painter, rect, bg, DRUM_KEY_LABELS[i], active);
